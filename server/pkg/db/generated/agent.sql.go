...
RETURNING task.id, task.agent_id, task.issue_id, task.status, task.priority, task.dispatched_at, task.started_at, task.completed_at, task.result, task.error, task.created_at, task.context, task.runtime_id, task.session_id, task.work_dir, task.trigger_comment_id, task.chat_session_id, task.autopilot_run_id, task.attempt, task.max_attempts, task.parent_task_id, task.failure_reason, task.trigger_summary, task.force_fresh_session, task.is_leader_task, task.wait_reason, task.initiator_user_id, task.handoff_note, task.prepare_lease_expires_at, task.squad_id, task.runtime_mcp_overlay, task.escalation_for_task_id, task.fire_at, task.originator_user_id, task.runtime_connected_apps, task.coalesced_comment_ids, task.delivered_comment_ids, task.chat_input_task_id, task.chat_finalize_deferred_at, task.originator_source, task.delegated_from_task_id, task.retry_of_task_id, task.rerun_of_task_id, task.rule_version_id, task.trigger_evidence_kind, task.trigger_evidence_ref_id, task.accountable_user_id, task.session_rollout_missing, task.retired_session_id, task.quick_actions_disabled, task.regenerate_quick_actions_for, task.branch_name, task.durable_work_dir, task.channel_context_revision, task.comment_thread_id, task.cancelled_by_type, task.cancelled_by_id, task.cancelled_by_name, task.routing_policy
`
      ...
      AND (
          CASE
              WHEN atq.routing_policy IS NULL
                  THEN atq.runtime_id = $1
              WHEN atq.routing_policy LIKE 'runtime:%'
                  THEN substr(atq.routing_policy, 9)::uuid = $1
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, comment_thread_id, cancelled_by_type, cancelled_by_id, cancelled_by_name, routing_policy FROM agent_task_queue
WHERE id = $1
`
...
const hasRoutedTaskForAgentAndRuntime = `-- name: HasRoutedTaskForAgentAndRuntime :one
SELECT EXISTS (
    SELECT 1
    FROM agent_task_queue atq
    WHERE atq.agent_id = $1
      ...
      AND (
          CASE
              WHEN atq.routing_policy LIKE 'runtime:%'
                  THEN substr(atq.routing_policy, 9)::uuid = $2::uuid
              WHEN atq.routing_policy LIKE 'provider:%'
                  THEN EXISTS (
                      SELECT 1 FROM agent_runtime pr
                      WHERE pr.id = $2::uuid
...
type HasRoutedTaskForAgentAndRuntimeParams struct {
	AgentID   pgtype.UUID `json:"agent_id"`
	RuntimeID pgtype.UUID `json:"runtime_id"`
}
...
func (q *Queries) HasRoutedTaskForAgentAndRuntime(ctx context.Context, arg HasRoutedTaskForAgentAndRuntimeParams) (bool, error) {
	row := q.db.QueryRow(ctx, hasRoutedTaskForAgentAndRuntime, arg.AgentID, arg.RuntimeID)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
SELECT atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.chat_session_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.trigger_summary, atq.force_fresh_session, atq.is_leader_task, atq.wait_reason, atq.initiator_user_id, atq.handoff_note, atq.prepare_lease_expires_at, atq.squad_id, atq.runtime_mcp_overlay, atq.escalation_for_task_id, atq.fire_at, atq.originator_user_id, atq.runtime_connected_apps, atq.coalesced_comment_ids, atq.delivered_comment_ids, atq.chat_input_task_id, atq.chat_finalize_deferred_at, atq.originator_source, atq.delegated_from_task_id, atq.retry_of_task_id, atq.rerun_of_task_id, atq.rule_version_id, atq.trigger_evidence_kind, atq.trigger_evidence_ref_id, atq.accountable_user_id, atq.session_rollout_missing, atq.retired_session_id, atq.quick_actions_disabled, atq.regenerate_quick_actions_for, atq.branch_name, atq.durable_work_dir, atq.channel_context_revision, atq.comment_thread_id, atq.cancelled_by_type, atq.cancelled_by_id, atq.cancelled_by_name, atq.routing_policy, rt.id AS match_runtime_id
FROM agent_task_queue atq
CROSS JOIN LATERAL (
    SELECT r.id
...
type ListQueuedClaimCandidatesByRuntimesRow struct {
	ID                        pgtype.UUID        `json:"id"`
	AgentID                   pgtype.UUID        `json:"agent_id"`
	IssueID                   pgtype.UUID        `json:"issue_id"`
	Status                    string             `json:"status"`
	...
	CancelledByName           pgtype.Text        `json:"cancelled_by_name"`
	RoutingPolicy             pgtype.Text        `json:"routing_policy"`
	MatchRuntimeID            pgtype.UUID        `json:"match_runtime_id"`
}
...
func (q *Queries) ListQueuedClaimCandidatesByRuntimes(ctx context.Context, runtimeIds []pgtype.UUID) ([]ListQueuedClaimCandidatesByRuntimesRow, error) {
	rows, err := q.db.Query(ctx, listQueuedClaimCandidatesByRuntimes, runtimeIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListQueuedClaimCandidatesByRuntimesRow{}
	for rows.Next() {
		var i ListQueuedClaimCandidatesByRuntimesRow
		if err := rows.Scan(
			&i.ID,
			&i.AgentID,
			&i.IssueID,
...
const resolveRoutingPolicyRuntime = `-- name: ResolveRoutingPolicyRuntime :one
SELECT r.id
FROM agent_runtime r
WHERE r.workspace_id = $1
  AND (
      CASE
          WHEN $2::text LIKE 'runtime:%'
              THEN r.id = substr($2::text, 9)::uuid
          WHEN $2::text LIKE 'provider:%'
              THEN r.provider = substr($2::text, 10)
          ELSE FALSE
      END
  )
...
type ResolveRoutingPolicyRuntimeParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
...
func (q *Queries) ResolveRoutingPolicyRuntime(ctx context.Context, arg ResolveRoutingPolicyRuntimeParams) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, resolveRoutingPolicyRuntime, arg.WorkspaceID, arg.Policy)
	var id pgtype.UUID
	err := row.Scan(&id)
	return id, err
}
