...
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, revision, last_activity_at, routing_policy, level
`
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, revision, last_activity_at, routing_policy, level FROM issue
WHERE workspace_id = $1
...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.properties, i.revision, i.last_activity_at, i.routing_policy, i.level FROM issue i
WHERE i.workspace_id = $1
  -- Negate only known terminal keys so an unknown legacy key remains active.
...
const setIssueLevel = `-- name: SetIssueLevel :one
UPDATE issue
SET level      = $1,
    updated_at = now()
WHERE id = $2 AND workspace_id = $3
...
type SetIssueLevelParams struct {
	Level       pgtype.Text `json:"level"`
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) SetIssueLevel(ctx context.Context, arg SetIssueLevelParams) (Issue, error) {
	row := q.db.QueryRow(ctx, setIssueLevel, arg.Level, arg.ID, arg.WorkspaceID)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const setIssueRoutingPolicy = `-- name: SetIssueRoutingPolicy :one
UPDATE issue
SET routing_policy = $1
WHERE id = $2
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, revision, last_activity_at, routing_policy, level
`

type SetIssueRoutingPolicyParams struct {
	RoutingPolicy pgtype.Text `json:"routing_policy"`
	ID            pgtype.UUID `json:"id"`
}
...
// inside it.
func (q *Queries) SetIssueRoutingPolicy(ctx context.Context, arg SetIssueRoutingPolicyParams) (Issue, error) {
	row := q.db.QueryRow(ctx, setIssueRoutingPolicy, arg.RoutingPolicy, arg.ID)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
WITH candidate AS (
    SELECT
        i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.properties, i.revision, i.last_activity_at, i.routing_policy, i.level,
